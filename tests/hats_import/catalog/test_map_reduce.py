...
def test_map_with_healpix_29(tmp_path, formats_dir, small_sky_single_file):
    ...
    mr.map_to_pixels(
        ...
        ra_column="NOPE",
        dec_column="NOPE",
        use_healpix_29=True,  # radec don't matter. just use existing index
        resume_path=tmp_path,
        mapping_key="map_0",
    )
    ...
    with pytest.raises(ValueError, match="columns expected but not found"):
        mr.map_to_pixels(
            ...
            ra_column="NOPE",
            dec_column="NOPE",
            use_healpix_29=True,  # no pre-existing index! expect failure.
            resume_path=tmp_path,
            mapping_key="map_0",
        )
...
def test_reduce_healpix_29(parquet_shards_dir, assert_parquet_file_ids, tmp_path):
    ...
    mr.reduce_pixel_shards(
        ...
        destination_pixel_size=131,
        output_path=tmp_path,
        add_healpix_29=False,  ## different from above
        ra_column="ra",
        dec_column="dec",
        sort_columns="id",
        delete_input_files=False,
...
@pytest.mark.parametrize("threshold_mode", ["row_count", "mem_size"])
def test_histogram_file_contents(tmp_path, small_sky_single_file, threshold_mode):
    """Test that map_to_pixels writes correct histogram file contents for both row_count and mem_size."""

    # Run mapping stage (with threshold_mode parameterized).
    mr.map_to_pixels(
        input_file=small_sky_single_file,
        pickled_reader_file=pickle_file_reader(tmp_path, get_file_reader("csv")),
        highest_order=0,
    ...
    result = read_partial_histogram(tmp_path, "map_0", which_histogram=threshold_mode)
    if threshold_mode == "row_count":
        assert len(result) == 12
        assert result[11] == 131  # known value for this fixture
        assert result.sum() == 131
    elif threshold_mode == "mem_size":
        assert len(result) == 12
        # Let's consider a very conservative lower bound for memory size.
        # Each row has at least 3 columns: id (int64), ra (float64), dec (float64).
        # Each int64 takes 8 bytes, each float64 takes 8 bytes.
        # So each row takes at least 8 + 8 + 8 = 24 bytes.
        # For 131 rows, minimum memory size is 131 * 24 = 3144 bytes.
        # However, since we are using a histogram with bins of size powers of two,
        # we expect the mem_size histogram to have a single bin that is at least 3144.
        assert result[11] >= 3144
        assert result.sum() >= 3144
...
def test_get_mem_size_of_chunk():
    ...
    mem_sizes_empty = mr._get_mem_size_of_chunk(empty_df)
    ...
    mem_sizes = mr._get_mem_size_of_chunk(df)
    ...
    mem_sizes_small = mr._get_mem_size_of_chunk(df_small)
    ...
    mem_sizes_table = mr._get_mem_size_of_chunk(table)
    ...
    mem_sizes_table_small = mr._get_mem_size_of_chunk(table_small)
...
def test_get_mem_size_of_chunk_nested():
    ...
    mem_sizes = mr._get_mem_size_of_chunk(nf)
