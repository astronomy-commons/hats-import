...
class ImportArguments(RuntimeArguments):
    ...
    ``pixel_threshold``. per-row sizes are estimates: fixed-length columns (numeric,
    boolean, datetime) are sized once from the first chunk of the first input file,
    and string/binary columns whose sampled sizes are consistent use their mean
    measured size from that chunk. this one estimate is shared by every input file,
    so all files are assumed to share a schema and similarly-sized values. other
    variable-length columns (e.g. lists, nested arrays), as well as string columns
    with wildly varying sizes (e.g. serialized arrays or free text), are measured
    row by row in every file."""
